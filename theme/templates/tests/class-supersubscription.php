...
<div class="scene i:scene tests">
	...
	<div class="tests">
		...
		$subscription = $SuperSubscriptionClass->addSubscription($item["item_id"], ["user_id" => $test_user_id]);
		...
		$subscription_duplet = $SuperSubscriptionClass->addSubscription($item["item_id"], ["user_id" => $test_user_id]);
		...
		$subscription = $SuperSubscriptionClass->getSubscriptions(["item_id" => $item["item_id"]])[0];
		if(
			$subscription &&
			$subscription["item_id"] == $item["item_id"] &&
			!$subscription["expires_at"] &&
		...
		if(
			...
			!$SuperSubscriptionClass->getSubscriptions(["item_id" => $item["item_id"]])
		): ?>
	...
	<div class="tests">
		...
		// $_POST["item_id"] = $item["item_id"];
		$subscription_duplet = $SuperSubscriptionClass->addSubscription($item["item_id"]);
		// unset($_POST);
		...
		<?
		// get the created subscription
		$subscription = $SuperSubscriptionClass->getSubscriptions(["item_id" => $item["item_id"], "user_id" => $test_user_id]);
		if(
			$subscription &&
			$subscription["item_id"] == $item["item_id"] &&
			$subscription["expires_at"] == $expires_at &&
		...
		if(
			$subscription && 
			$SuperSubscriptionClass->deleteSubscription($subscription["id"]) &&
			!$SuperSubscriptionClass->getSubscriptions(array("item_id" => $item["item_id"]))
		): ?>
		<div class="testpassed"><p>SuperSubscription::deleteSubscription, item without price or subscription method - correct</p></div>
		<? else: ?>
		<div class="testfailed"><p>SuperSubscription::deleteSubscription, item without price or subscription method - error</p></div>
	...
	<div class="tests">
		...
		// item without price – should succeed
		// $_POST["item_id"] = $item["item_id"];
		$subscription = $SuperSubscriptionClass->addSubscription($item["item_id"]);
		// unset($_POST);
	...
	<div class="tests">
		...
		$price = $model_tests->addPrice(array("addPrice", $item["item_id"]));
		...
		<?
		// get the created subscription
		$subscription = $SuperSubscriptionClass->getSubscriptions(array("item_id" => $item["item_id"]));
		if(!$subscription): ?>
