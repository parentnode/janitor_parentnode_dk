...
<div class="scene i:scene tests">
	...
	<div class="tests update">
		...
		if($test_save_simple && $reference_item && $test_save_simple === $reference_item) {
			...
			if($test_update_simple &&
				...
				strpos($reference_item_updated["sindex"], superNormalize($reference_item_updated["name"])) === 0 &&
			...
			list($variant1, $variant2) = array_keys($reference_item_updated["mediae"]);

			// debug([$reference_item_updated, message()->getMessages()]);
			...
			if($test_update_files &&
				...
				$reference_item_updated["mediae"][$variant1]["name"] === $_FILES["v_files"]["name"][1] &&
				$reference_item_updated["mediae"][$variant1]["format"] === "jpg" &&
				$reference_item_updated["mediae"][$variant2]["name"] === $_FILES["v_files"]["name"][0] &&
				$reference_item_updated["mediae"][$variant2]["format"] === "png" &&
			...
			$_FILES["v_files"] = [
				"type" => ["image/png", "image/jpeg"],
				"name" => ["Test 2 file png", "Test 2 file jpg"],
				"tmp_name" => [LOCAL_PATH."/templates/tests/file_upload/test-running.png", LOCAL_PATH."/templates/tests/file_upload/test-running.jpg"],
				"error" => ["", ""]
			];
			...
			if($test_update_files &&
				$reference_item_updated["mediae"] &&
				count($reference_item_updated["mediae"]) === 5 &&
	...
	<div class="tests delete">
		...
		copy(LOCAL_PATH."/templates/tests/file_upload/test.png", LOCAL_PATH."/templates/tests/file_upload/test-running.png");
		copy(LOCAL_PATH."/templates/tests/file_upload/test.jpg", LOCAL_PATH."/templates/tests/file_upload/test-running.jpg");
		$_POST["name"] = "Test complex item, multiple files, delete";
		$_FILES["v_files"] = [
			"type" => ["image/png", "image/jpeg"],
			"name" => ["Test file png", "Test file jpg"],
			"tmp_name" => [LOCAL_PATH."/templates/tests/file_upload/test-running.png", LOCAL_PATH."/templates/tests/file_upload/test-running.jpg"],
	...
	<div class="tests addMedia">
		...
		if($uploads &&
			...
			$uploads[0]["height"] === 288 &&
			$uploads[0]["filesize"] === filesize(LOCAL_PATH."/templates/tests/file_upload/test.png") &&
			preg_match("/v_files\-/", $uploads[0]["variant"]) &&
	...
	<div class="tests addSingleMedia">
		...
		if($uploads &&
			...
			$uploads["name"] === $_FILES["v_file"]["name"][0] &&
			$uploads["format"] === "png" &&
			$uploads["width"] === 512 &&
			$uploads["height"] === 288 &&
			$uploads["filesize"] === filesize(LOCAL_PATH."/templates/tests/file_upload/test.png") &&
			$uploads["variant"] === "v_file" &&

			!file_exists("templates/tests/file_upload/test-running.png") &&
			!file_exists(PUBLIC_FILE_PATH . "/" . $test_upload["id"]) &&

			file_exists(PRIVATE_FILE_PATH . "/" . $test_upload["id"] . "/".$uploads["variant"]."/png") &&
