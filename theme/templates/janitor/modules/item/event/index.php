...
<div class="scene module i:module pageitem i:pageitem">
	...
	<div class="controllers">
		...
		<p>
			Controllers are used to access event items and create meaningful urls on your website.
		</p>
		<p>
			Your event items can be accessed via any of your event item controllers by adding the event item sindex to the 
			controller path, like this: .
		</p>
		<code><?= SITE_URL ?>/events/page-item-sindex</code>

		<h3>Existing event item controllers</h3>
		<ul class="controllers" 
			data-csrf-token="<?= session()->value("csrf") ?>"
			data-delete-action="<?= security()->validPath(HTML()->path."/modules/deleteController/item/event") ?>"
		...
		<?= HTML()->formStart("modules/addController/item/event", array("class" => "new labelstyle:inject")) ?>
			<fieldset>
