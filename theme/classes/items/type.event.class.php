...
class TypeEvent extends Itemtype {
	...
	*/
	function __construct() {
		...
		// Ticket url
		$this->addToModel("ticket_url", array(
			"type" => "string",
			"label" => "Ticket url",
			"pattern" => "^http[s]?\:\/\/[^$]+",
			"hint_message" => "Link to ticket platform or page.",
			"error_message" => "Link must start with http:// or https://.",
		));

		// Ticket price
		$this->addToModel("ticket_price", array(
			"type" => "number",
			"label" => "Ticket price",
			"hint_message" => "Ticket price in ".DEFAULT_CURRENCY_ISO.".",
			"error_message" => "Ticket price must be a valid number.",
		));
	...
	function saved($item_id) {
		...
		// Update starting date to next day, to avoid event ending up in past grouping
